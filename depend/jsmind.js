...
; (function ($w) {
    ...
    jm.mind.prototype = {
        ...
        _move_node: function (node, beforeid, parentid, direction) {
            if (!!node && !!parentid) {
                ...
                window.postMessage('move_node#' + node.id + '#' + beforeid, '*');
    ...
    jm.prototype = {
        ...
        click_handle: function (e) {
            ...
            // if (element.innerHTML.indexOf('jmnode') === -1 && element.innerHTML.indexOf('http') !== -1) {
            //    window.open(element.innerHTML,'_blank');
            // }
            if (isexpander) {
                var nodeid = this.view.get_binded_nodeid(element);
                if (!!nodeid) {
    ...
    jm.view_provider.prototype = {
        ...
        edit_node_begin: function (node) {
            if (!node.topic) {
                window.postMessage('edit_image_nodes#' + node.data.backgroundImage, '*');
                return;
            }
            if (this.editing_node != null) {
                this.edit_node_end();
            }
            window.postMessage('edit_node_begin#' + node.topic, '*');
        ...
        edit_node_end: function () {
            if (this.editing_node != null) {
                ...
                window.postMessage('edit_node_end#' + node.id, '*');
            }
        },
