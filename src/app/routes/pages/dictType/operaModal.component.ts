    ...
    <nz-modal [(nzVisible)]="Visible" nzWidth="700" [nzTitle]="popTitle" (nzOnCancel)="Cancel()" (nzOnOk)="Ok()">
        <div *ngIf="operaType !== 'del'" nz-row>
            <div nz-col nzSpan="12">
                <nz-form-item>
                    <nz-form-label nzRequired [nzSpan]="8">类型名称</nz-form-label>
                    <nz-form-control [nzSpan]="12"> <input nz-input [(ngModel)]="nameValue" /> </nz-form-control>
                </nz-form-item>
            </div>
            <div nz-col nzSpan="12">
                <nz-form-item>
                    <nz-form-label nzRequired [nzSpan]="8">类型编码</nz-form-label>
                    <nz-form-control [nzSpan]="12"> <input nz-input [(ngModel)]="typeValue" /> </nz-form-control> 
                </nz-form-item>
            </div>
            <div nz-col nzSpan="12">
                <nz-form-item>
...
export class OperaModalComponent implements OnInit {
    ...
    ngOnInit() {
        switch (this.operaType) {
            case 'add':
                this.popTitle = '新增字典';
                this.okUrl = environment.apiBase + '/system/dictType/add';
                setTimeout(() => {
                    this.Visible = true;
                    this.getType(() => {
                        // to do
            ...
            case 'edit':
                this.popTitle = '修改字典';
                this.okUrl = environment.apiBase + '/system/dictType/update';
                ...
                if (editstatus && editcount === 1) {
                    ...
                    this.getType(() => {
                        ...
                        this.typeValue = this.operaData.type;
                        this.orderValue = JSON.stringify(this.operaData.nOrder);
            ...
            case 'del':
                this.popTitle = '删除字典';
                this.okUrl = environment.apiBase + '/system/dictType/delete';
                let delstatus = false;
                let delcount = 0;
                for (const i in this.responseData) {
                    if (this.responseData[i].checked) {
    ...
    Ok() {
        ...
        switch (this.operaType) {
            case 'add':
                if (this.nameValue === '') {
                    this.msg.error('类型名称不能为空');
                    return;
                }
                if (!this.typeValue || this.typeValue === '') {
                    this.msg.error('类型编码不能为空');
                    return;
                }
                param = { 'name': this.nameValue, 'type': this.typeValue, 'nOrder': this.orderValue, 'remark': this.remarkValue };
                break;
            case 'edit':
                if (this.nameValue === '') {
                    this.msg.error('类型名称不能为空');
                ...
                param = { id: this.operaId, 'name': this.nameValue, 'type': this.typeValue, 'nOrder': this.orderValue, 'remark': this.remarkValue };
                break;
            case 'del':
