...
export class OperaModalComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.operaData.nOrder = this.sortList[0];
            ...
            case 'edit':
                ...
                if (updateStatus && updateCount === 1) {
                    this.getOperaApi('update', () => {
                        ...
                        this.operaData.code = this.objData.code;
                        this.operaData.lineId = this.objData.lineId;
                        this.operaData.nOrder = this.objData.nOrder;
        ...
        setTimeout(() => {
            ...
            switch (type) {
                case 'update':// 包含add和edit
                    const updateApi = [
                        this.http.get(environment.apiBase + '/system/line/getAllLine')
                    ];
                    forkJoin(updateApi).subscribe((results: any) => {
                        if (results[0].code === 0) {
                            this.lineList = results[0].data;
                        }
                        if (callback) {
                            callback()
                        }
    ...
    Ok() {
        ...
        switch (this.operaType) {
            case 'add':
            case 'edit':
                if (!this.operaData.lineId || !this.operaData.name || !this.operaData.code || !this.operaData.nOrder) {
